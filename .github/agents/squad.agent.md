...
> **0. Check `SQUAD_HQ_ROOT` env var first.** If set, use that path as TEAM_ROOT immediately — skip git-based discovery entirely.
>    - This is "deployed mode": the HQ repo (`clonewars-squad-hq`) is separate from the work repo.
>    - All `.squad/` reads and writes go to the HQ path. Work repo commits go to the work repo root.
>    - If `SQUAD_HQ_ROOT` is not set, continue to step 1 (existing behavior — fully backward compatible).
...
- Also pass `HQ_ROOT: {hq_root}` (same as TEAM_ROOT in local mode; HQ path in deployed mode) and `WORK_ROOT: {work_root}` (always the current work repo root from `git rev-parse`).
...
prompt: |
  ...
  TEAM ROOT: {team_root}
  CURRENT_DATETIME: {current_datetime}
  HQ_ROOT: {hq_root}      → HQ repo path (same as TEAM_ROOT in local mode)
  WORK_ROOT: {work_root}  → current work repo root (for non-.squad/ commits)
  ...
  7. GIT COMMIT (two-root aware): Stage only the exact `.squad/` files Scribe wrote in this session. Use `git status --porcelain` (run via `git -C {HQ_ROOT}`) filtered to allowed paths (decisions.md, decisions-archive.md, agents/{name}/history.md, agents/{name}/history-archive.md, log/*, orchestration-log/*). Stage each file individually with `git -C {HQ_ROOT} add -- <path>`. Handle renames by extracting destination path (`-replace '^.* -> ',''`). Commit with -F (write msg to temp file) using `git -C {HQ_ROOT} commit -F {temp-file}`. For work repo changes (non-.squad/ files): `git -C {WORK_ROOT} add <files> && git -C {WORK_ROOT} commit -F {temp-file}`. In local mode (HQ_ROOT == WORK_ROOT), these are the same repo — single commit as before. Skip if nothing staged in either root. ⚠️ NEVER use `git add .squad/` or broad globs.
